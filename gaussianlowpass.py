...
def main():
    ...
    filtered = gaussianfilter(img,args.masksize);

    plt.figure("Gaussing low Pass filter")
    ...
    noisyfiltered = gaussianfilter(noisyimage, args.masksize)
...
def gaussianfilter(image,masksize):
    mask = cv.getGaussianKernel(masksize, sigma = -1)
